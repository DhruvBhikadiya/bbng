...
const customers = {
    create: async (data) => {
        ...
        try {
            ...
            const sql = 'INSERT INTO customers (email, mobile, name, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())';
            const [results] = await db.execute(sql, [data.email, data.mobile, data.name]);
    ...
    update: async (id, data) => {
        ...
        try {
            ...
            const sql = 'UPDATE customers SET email = ?, mobile = ?, name = ?, updated_at = NOW() WHERE id = ?';
            const [results] = await db.execute(sql, [data.email, data.mobile, data.name, id]);
